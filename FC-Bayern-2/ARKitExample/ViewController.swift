...
class ViewController: UIViewController, ARSCNViewDelegate, UIPopoverPresentationControllerDelegate, VirtualObjectSelectionViewControllerDelegate, SCNSceneRendererDelegate {
    ...
    func doFaceDetection(){
        if let arFrame = sceneView.session.currentFrame {
            ...
            do {
                try imageRequestHandler.perform([self.initFaceDetectionRequest(frame: arFrame)])
            } catch {
                print(error)
            }
        ...
        print(request)
            ...
            for observation in result {
                if let observation = observation {
                    ...
                    let hitResultTopLeftArray = frame.hitTest(topLeft, types: [.featurePoint])
                    let hitResultTopRightArray = frame.hitTest(topRight, types: [.featurePoint])
                    let hitResultBottomLeftArray = frame.hitTest(bottomLeft, types: [.featurePoint])
                    let hitResultBottomRightArray = frame.hitTest(bottomRight, types: [.featurePoint])
                    guard let hitResultTopLeft = hitResultTopLeftArray.first else {
                        continue
                    }
                    guard let hitResultTopRight = hitResultTopRightArray.first else {
